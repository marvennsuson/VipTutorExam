...
@include('layouts.notification')
    <div class="container">
        <div class="col-lg-12">
            <form id="create_product" action="{{ route('admin.product.update', $record["id"]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                <div class="row justify-content-start">
                    ...
                    <div class="col-sm-8 shadow ">
                        <div class=" p-3">
                            ...
                            <!-- Uploaded image area-->
                            <div class="image-area mt-4"><img id="imageResult" src="{{ !empty($record["image"]) && Route::is('admin.product.show')  ? $record["fullpath"] . '/' . $record["image"]  : '#' }}" alt=""
                            ...
                            <div class="form-group w-50 mb-3">
                                ...
                                <input type="text" class="form-control " name="title" placeholder="Title here" id="src" value="{{ $record["title"] }}">
                            </div>
                            <div class="input-group shadow-sm rounded py-2">
                                ...
                                <textarea id="editor" class="form-control b-none" name="description" style="height: 100px;"
                                    placeholder="Type Content Here">{{ $record["description"] }}</textarea>
                            </div>
    
                            <div class="mt-4">
                            <h6>Price</h6>
                            <input type="text" class="form-control b-none" name="price" id="currency-field" pattern="^\₱\d{1,3}(,\d{3})*(\.\d+)?₱" value="{{ $record["price"] }}" data-type="currency" placeholder="₱ 00.00">
                        </div>
                            <div class="mt-4">
                                <h6>Stock</h6>
                                <input type="text" class="form-control b-none" name="stock" id="stock" value="{{ $record["stock"] }}" />
                            </div>
                        </div>
                        <div class="d-flex justify-content-start  p-3">
                            <button type="submit" class="btn btn-success form-control w-25 mr-3">Update Product</button>
                            &nbsp;
                            <a href="{{ route('admin.product.index') }}" class="btn btn-danger form-control w-25">cancel</a>
                        </div>
                    </div>
